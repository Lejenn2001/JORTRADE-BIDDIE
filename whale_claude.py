...
Fetches real market intelligence from Unusual Whales and runs AI analysis with Claude.
...
Usage:
  ...
  python whale_claude.py stock NVDA                  # Full stock deep-dive
"""
...
def uw_get(path: str, params: dict = None) -> dict | list | None:
    ...
    try:
        ...
        if r.status_code == 404:
            console.print(f"[yellow]Endpoint not available:[/] {path}")
        else:
...
def fetch_flow_for_ticker(ticker: str, all_alerts: list) -> list:
    """Filter flow alerts by ticker from a pre-fetched list."""
...
Examples:
  ...
  python whale_claude.py flow --limit 100
  python whale_claude.py market
  python whale_claude.py darkpool NVDA
  python whale_claude.py stock TSLA
  python whale_claude.py stock AAPL --limit 50
        """,
    )
    ...
    subparsers.add_parser("market", help="Market overview: sectors, economic & FDA calendar")
