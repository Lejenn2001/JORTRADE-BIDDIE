...
**Closed-market snapshot fallback:** snapshot JSON returns zeros (`lastTrade.p`, `day.c`, `updated` = 0) on weekends/holidays (e.g. Juneteenth Jun 19) — only `prevDay.c` has data. Must use `||` not `??` (zeros are present, not null, so `??` won't fall through): `t.lastTrade?.p || t.day?.c || t.prevDay?.c`. Otherwise the ticker goes blank when the market is closed instead of showing last close.
