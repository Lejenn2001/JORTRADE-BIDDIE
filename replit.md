...
- **Signal Categories**: 3 categories — Algorithm Plays (price action + gamma analysis), Whale Plays ($250K+ institutional flow), Spreads & Butterflies (multi-leg strategies). Dashboard shows ONLY Algorithm Plays (top 5, sorted 0DTE first then conviction score). Signals page has 3 tabs for all categories. Each signal card labeled "DAY TRADE" (buy_now/short_term) or "SWING TRADE" (swing). Example signals fill tabs when no live data (META/AMZN/GOOGL for spreads, AAPL/PLTR/AMD for whale).
