...
- **Chat Trade Action Bar (Biddie chat panel):** Each Biddie assistant message is parsed for option contracts ("TICKER STRIKE[CP] M/D[/YY]") via `extractContractsFromText()` in `src/lib/extractContracts.ts`. Detected contracts render an action bar labeled "Chat Idea — Not Execution Approved" with three buttons per contract: Buy Paper, Monitor, Review. Buy Paper / Review open the existing PaperTradeTicket modal in chat-trade mode (`source: "chat"`), which shows an amber "Chat Trade (Unverified)" banner, suppresses the signal plan card, and replaces "Recommended Contract" / "Highest-probability" copy with neutral "As Quoted in Chat" / "Alternate Contract" labels. The execution evaluator is never called for chat trades. Monitor persists locally in `localStorage` key `biddie-chat-monitor-${userId}` (no schema change). Parser anchors year resolution to `msg.createdAt` so an expiry like "5/1" never silently rolls forward to the next year on later renders. Backend `POST /whale/paper/trades` accepts the synthetic `chat-...` signalId as plain TEXT (no FK lookup).
