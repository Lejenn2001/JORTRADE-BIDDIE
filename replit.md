...
- **Direction Stability Lock**: Prevents thesis direction from flip-flopping between CALL/PUT on each rescan. Uses in-memory direction lock per ticker with 30-min TTL. Lock threshold=25 (min score to establish direction), flip threshold=35 (min score to reverse an existing lock). Confirmed breakouts always override the lock. Neutral tickers (score <25) show no contract rec
