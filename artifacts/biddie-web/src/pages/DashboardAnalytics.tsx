...
const DashboardAnalytics = () => {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "mytrades" | "signals" | "pnl" | "admin">("overview");
          ...
          <div className="max-w-6xl mx-auto space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex gap-1 bg-muted/30 rounded-lg p-1">
                {(["overview", "mytrades", "pnl", "signals", ...(isAdmin ? ["admin" as const] : [])] as const).map(tab => (
                  <button
                    key={tab}
                  ...
                  >
                    {tab === "overview" ? "Overview" : tab === "mytrades" ? "My Trades" : tab === "pnl" ? "P&L Calendar" : tab === "admin" ? "Admin" : "AI Signals"}
                  </button>
                ))}
              </div>
            </div>
