...
const Dashboard = () => {
  ...
  const algorithmPlays = useMemo(() =>
    sortSignals(
      allMergedSignals
        .filter(s => (s.category === 'algorithm' || (s.category !== 'whale' && s.category !== 'spread')) && getSignalScore(s) >= 50)
    ).slice(0, 5),
    [allMergedSignals]
  );
  ...
  const whalePlays = useMemo(() =>
    sortSignals(
      allMergedSignals
        .filter(s => s.category === 'whale' && getSignalScore(s) >= 50)
    ).slice(0, 5),
    [allMergedSignals]
  );
  ...
  const spreadPlays = useMemo(() =>
    sortSignals(
      allMergedSignals
        .filter(s => s.category === 'spread' && getSignalScore(s) >= 50)
    ).slice(0, 5),
    [allMergedSignals]
  );
