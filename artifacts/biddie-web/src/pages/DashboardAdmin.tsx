            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              {showReference && (
                ...
                >
                  <div className="glass-panel rounded-xl p-6 border-border/40">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3 mb-4">
                      ...
                      <div className="rounded-lg bg-amber-500/10 border border-amber-500/30 p-4">
                        ...
                        <p className="text-xs text-muted-foreground leading-relaxed">Still active — hasn't expired, target not hit, invalidation not breached. Excluded from win rate.</p>
                      </div>
                  ...
                  <div className="glass-panel rounded-xl p-6 border-border/40">
                    <h3 className="text-lg font-bold text-foreground mb-4">Signal Categories — Detailed Algorithms</h3>

                    <div className="space-y-4">
                      <div className="bg-card/50 rounded-lg p-4 border border-emerald-500/20">
                        <h4 className="font-semibold text-emerald-400 mb-2">Algorithm Plays</h4>
                        <p className="text-xs text-muted-foreground mb-3">Default category for signals that pass all hard filters but don't meet Whale or Spread thresholds. Single-leg directional plays confirmed by price action, gamma zones, and ICT structure.</p>
                        <div className="text-xs text-muted-foreground space-y-1.5">
                          <p className="text-foreground font-semibold text-[11px]">Hard Filters (must pass all):</p>
                          <p>• Minimum <span className="text-emerald-400 font-medium">$25,000</span> total premium</p>
                          <p>• Minimum <span className="text-emerald-400 font-medium">50%</span> ask-side aggression</p>
                          <p>• Strike within <span className="text-emerald-400 font-medium">15% OTM</span> max (deep OTM rejected)</p>
                          <p>• Strike not deeper than <span className="text-emerald-400 font-medium">5% ITM</span> (likely hedge — rejected)</p>
                          <p>• Expiry within <span className="text-emerald-400 font-medium">45 days</span> (far-out rejected)</p>
                          <p>• Polygon live price vs UW price can't diverge more than <span className="text-emerald-400 font-medium">20%</span></p>
                          <p>• Strike/price ratio must be between <span className="text-emerald-400 font-medium">0.33x – 3x</span></p>
                          <p className="text-foreground font-semibold text-[11px] pt-2">Price Action Confirmation:</p>
                          <p>• Checks 1m/5m candles for bullish/bearish engulfing, pin bars, or breaks of VWAP/PDH/PDL/Pivots</p>
                          <p>• Gamma zone alignment (negative gamma = explosive moves, positive gamma = mean reversion)</p>
                          <p>• VWAP position: Calls above VWAP or Puts below VWAP = "⚡ Act Now" tag</p>
                        </div>
                      </div>

                      <div className="bg-card/50 rounded-lg p-4 border border-blue-500/20">
                        <h4 className="font-semibold text-blue-400 mb-2">Whale Plays</h4>
                        <p className="text-xs text-muted-foreground mb-3">Large institutional flow. Same hard filters as Algorithm, plus premium size thresholds that separate institutional positioning from retail noise.</p>
                        <div className="text-xs text-muted-foreground space-y-1.5">
                          <p className="text-foreground font-semibold text-[11px]">Classification (either condition):</p>
                          <p>• Total premium ≥ <span className="text-blue-400 font-medium">$2,000,000</span></p>
                          <p>• OR total premium ≥ <span className="text-blue-400 font-medium">$1,000,000</span> AND (order is a <span className="text-blue-400 font-medium">Sweep</span> OR ask aggression ≥ <span className="text-blue-400 font-medium">90%</span>)</p>
                          <p className="text-foreground font-semibold text-[11px] pt-2">Why it matters:</p>
                          <p>• $1M+ sweeps with 90%+ ask aggression = someone is urgently building a large directional position</p>
                          <p>• $2M+ single-leg = institutional conviction regardless of execution style</p>
                        </div>
                      </div>

                      <div className="bg-card/50 rounded-lg p-4 border border-purple-500/20">
                        <h4 className="font-semibold text-purple-400 mb-2">Spread Plays</h4>
                        <p className="text-xs text-muted-foreground mb-3">Multi-leg options strategies with defined risk. Detected via Unusual Whales' multi-leg flag.</p>
                        <div className="text-xs text-muted-foreground space-y-1.5">
                          <p className="text-foreground font-semibold text-[11px]">Detection (two paths):</p>
                          <p>• UW <span className="text-purple-400 font-medium">has_multileg = true</span> flag on the flow alert</p>
                          <p>• OR AI-generated spread recommendations from strong Whale/Algorithm signals (Claude identifies optimal spread structures)</p>
                          <p>• Indicates vertical spreads, iron condors, butterflies, or other multi-leg structures</p>
                          <p>• Same hard filters applied (premium, aggression, moneyness)</p>
                        </div>
                      </div>

                      <div className="bg-card/50 rounded-lg p-4 border border-amber-500/20">
                        <h4 className="font-semibold text-amber-400 mb-2">Biddie Picks ⭐</h4>
                        <p className="text-xs text-muted-foreground mb-3">The highest-conviction signals. Must pass both algorithmic scoring AND Claude AI evaluation with ICT/SMC analysis.</p>
                        <div className="text-xs text-muted-foreground space-y-1.5">
                          <p className="text-foreground font-semibold text-[11px]">All 3 must be true:</p>
                          <p>• Claude AI confirms it is <span className="text-amber-400 font-medium">NOT a hedge</span> (is_hedge = false)</p>
                          <p>• AI-adjusted confidence score ≥ <span className="text-amber-400 font-medium">8 out of 10</span></p>
                          <p>• Signal quality rated <span className="text-amber-400 font-medium">"strong" or "moderate"</span> by Claude</p>
                          <p className="text-foreground font-semibold text-[11px] pt-2">Claude AI evaluation criteria (ICT/SMC):</p>
                          <p>• <span className="text-foreground font-medium">8-10 score:</span> Trend-aligned flow + buying at Discount (calls) or Premium (puts) + liquidity sweep detected</p>
                          <p>• <span className="text-foreground font-medium">5-8 score:</span> Valid reversal with CHoCH (Change of Character) confirmed, or Fair Value Gap play</p>
                          <p>• <span className="text-foreground font-medium">3-5 score:</span> Counter-trend without structural support, or index puts on green days (likely hedges)</p>
                          <p>• Claude checks for: BOS (Break of Structure), CHoCH, FVG (Fair Value Gaps), Order Blocks, Liquidity Sweeps</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="glass-panel rounded-xl p-6 border-border/40">
                    <h3 className="text-lg font-bold text-foreground mb-4">Confidence Scoring — Base Algorithm</h3>
                    <p className="text-xs text-muted-foreground mb-4">Every signal starts at a base score of <span className="text-foreground font-semibold">5</span>. Points are added or subtracted based on flow quality, then the top 15 candidates go to Claude for final AI refinement.</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div className="bg-card/50 rounded-lg p-3 border border-emerald-500/20">
                        <p className="text-[10px] font-bold text-emerald-400 uppercase mb-2">Score Additions</p>
                        <div className="text-xs text-muted-foreground space-y-1">
                          <p><span className="text-emerald-400 font-medium">+1</span> — Sweep order (aggressive across exchanges)</p>
                          ...
                          <p><span className="text-emerald-400 font-medium">+0.5</span> — Vol/OI ratio ≥ 2x</p>
                          <p><span className="text-emerald-400 font-medium">+1</span> — Vol/OI ratio ≥ 5x</p>
                          <p><span className="text-emerald-400 font-medium">+0.5 to +1</span> — ATM strike (within 2-5%)</p>
                          <p><span className="text-emerald-400 font-medium">+1</span> — Price action confirmation (engulfing, pin bar, level break)</p>
                        </div>
                      </div>
                      <div className="bg-card/50 rounded-lg p-3 border border-red-500/20">
                        ...
                        <div className="text-xs text-muted-foreground space-y-1">
                          <p><span className="text-red-400 font-medium">-1.5</span> — Counter-trend (e.g. Call below VWAP, Put above VWAP)</p>
                        ...
                        <div className="text-xs text-muted-foreground space-y-1">
                          <p>• Claude adjusts confidence based on ICT/SMC confluence</p>
                          <p>• Returns: is_hedge, adjusted_confidence, signal_quality</p>
                          <p>• Scores map to display: 6→70, 7→78, 8→85, 9→92</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="glass-panel rounded-xl p-6 border-border/40">
                    <h3 className="text-lg font-bold text-foreground mb-4">Breakout Scanner — Detection Algorithm</h3>
                    <p className="text-xs text-muted-foreground mb-4">Scans 40+ tickers using 60 days of daily candles + live Polygon WebSocket data. Results cached for 5 minutes.</p>
                    <div className="space-y-3">
                      <div className="bg-card/50 rounded-lg p-3 border border-border/30">
                        <p className="text-[10px] font-bold text-cyan-400 uppercase mb-2">Detection Layers & Scoring</p>
                        <div className="text-xs text-muted-foreground space-y-1">
                          <p><span className="text-cyan-400 font-medium">+25 pts</span> — <span className="text-foreground font-medium">Squeeze:</span> Bollinger Bands (2.0 std dev) inside Keltner Channels (1.5 ATR) + bonus for squeeze length</p>
                          <p><span className="text-cyan-400 font-medium">+15 pts</span> — <span className="text-foreground font-medium">Consolidation:</span> Price within 3% range for 3+ days, support/resistance from 20-day high/low frequency</p>
                          <p><span className="text-cyan-400 font-medium">+20 pts</span> — <span className="text-foreground font-medium">Volume spike:</span> Current volume &gt; 2.0x 20-day average (+10 for &gt; 1.3x)</p>
                          <p><span className="text-cyan-400 font-medium">+trend</span> — <span className="text-foreground font-medium">Momentum:</span> EMA8 rising + price above SMA20 = bullish; EMA8 falling + below SMA20 = bearish</p>
                          <p><span className="text-cyan-400 font-medium">+flow</span> — <span className="text-foreground font-medium">Options flow bias:</span> Call/put premium ratios, sweep activity, ask-side aggression. Near-term (0-7 day) flows weighted 1.0-1.5x</p>
                        </div>
                      </div>
                      <div className="bg-card/50 rounded-lg p-3 border border-border/30">
                        <p className="text-[10px] font-bold text-amber-400 uppercase mb-2">Breakout Trigger ("ENTER NOW")</p>
                        <div className="text-xs text-muted-foreground space-y-1">
                          <p>1. Current candle <span className="text-foreground font-medium">closes above resistance</span> (bullish) or <span className="text-foreground font-medium">below support</span> (bearish)</p>
                          <p>2. Previous candle was still <span className="text-foreground font-medium">inside</span> the range</p>
                          <p>3. Volume must be <span className="text-amber-400 font-medium">≥ 1.5x</span> the 20-day average</p>
                        </div>
                      </div>
                      <div className="bg-card/50 rounded-lg p-3 border border-border/30">
                        <p className="text-[10px] font-bold text-red-400 uppercase mb-2">Real-Time Alert Generation (WebSocket)</p>
                        <div className="text-xs text-muted-foreground space-y-1">
                          <p>• Live price crosses resistance/support buffer (<span className="text-foreground font-medium">0.2%</span>) via Polygon WebSocket</p>
                          <p>• Session volume ratio must be <span className="text-red-400 font-medium">≥ 1.5x</span> (vs expected for time of day)</p>
                          <p>• Burst volume ratio must be <span className="text-red-400 font-medium">≥ 3.0x</span> (last 60s vs last 10 min)</p>
                          <p>• Both high → tagged <span className="text-foreground font-medium">"INSTITUTIONAL"</span></p>
                          <p>• <span className="text-foreground font-medium">15-minute cooldown</span> per ticker/direction</p>
                          <p>• Auto-suggests specific option contract (strike, expiry, type)</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="glass-panel rounded-xl p-6 border-border/40">
                    <h3 className="text-lg font-bold text-foreground mb-4">Dashboard Biddie — Chat AI</h3>
                    <p className="text-xs text-muted-foreground mb-4">Claude Sonnet 4 (claude-sonnet-4-6) with real-time data injection. Two modes: full analysis (private chat) and social (community chat).</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div className="bg-card/50 rounded-lg p-3 border border-border/30">
                        <p className="text-[10px] font-bold text-purple-400 uppercase mb-2">Data Available Per Response</p>
                        <div className="text-xs text-muted-foreground space-y-1">
                          <p>• Live UW options flow (premium, sweeps, ask/bid aggression %) — <span className="text-foreground font-medium">always injected</span></p>
                          <p>• VWAP (5-min calc), Pivot Points (R1, R2, S1, S2), PDH/PDL — <span className="text-foreground font-medium">per ticker mentioned</span></p>
                          <p>• Market structure: FVGs, Order Blocks, Liquidity Levels — <span className="text-foreground font-medium">per ticker mentioned</span></p>
                          <p>• Dark pool activity (off-exchange prints) — <span className="text-foreground font-medium">when ticker requested or "darkpool" mentioned</span></p>
                          <p>• Sector ETF performance + economic calendar — <span className="text-foreground font-medium">when "market" sentiment detected</span></p>
                          <p>• 14-day trading calendar with MWF expiry schedules — <span className="text-foreground font-medium">always injected</span></p>
                        </div>
                      </div>
                      <div className="bg-card/50 rounded-lg p-3 border border-border/30">
                        <p className="text-[10px] font-bold text-purple-400 uppercase mb-2">Trade Call Format (Structured)</p>
                        <div className="text-xs text-muted-foreground space-y-1">
                          <p>• Ticker + specific contract (e.g. "SPY 645 Put")</p>
                          <p>• Entry trigger tied to VWAP/key levels</p>
                          <p>• Target price + Invalidation level</p>
                          <p>• Confidence score (7-10) from flow aggression + premium</p>
                          <p>• Position reviews: Delta, Gamma triggers, Theta decay → Hold/Cut/Add</p>
                        </div>
                      </div>
                      <div className="bg-card/50 rounded-lg p-3 border border-border/30 md:col-span-2">
                        <p className="text-[10px] font-bold text-amber-400 uppercase mb-2">Personality & Guardrails</p>
                        <div className="text-xs text-muted-foreground space-y-1">
                          <p>• Personality: "Your trading bestie" — warm, casual, trader slang ("tbh", "lowkey", "0dte", "whale")</p>
                          <p>• Never suggests alternative tickers — only answers what's asked</p>
                          <p>• Validates expiry dates against real trading calendar</p>
                          <p>• If nothing good: "Honestly? Not much worth looking at right now. Sit tight, don't force it."</p>
                          <p>• <span className="text-foreground font-medium">/whale/chat</span> — full data injection (private dashboard)</p>
                          <p>• <span className="text-foreground font-medium">/whale/community-chat</span> — lighter social mode unless asked for breakdown</p>
                        </div>
                      </div>
                    </div>
                  </div>
